import time
from random import randint
from typing import List
from typing import Set
import matplotlib.pyplot as plt
from convex_hull import Point
from convex_hull import base_case_hull
from convex_hull import compute_hull

def generate_points(
        num_points: int,
        min_x: int = 0,
        max_x: int = 1_000,
        min_y: int = 0,
        max_y: int = 1_000,
) -> List[Point]:
    """ Creates a list of random and unique points for benchmarking the convex_hull algorithm.
    :param num_points: number of unique points to generate.
    :param min_x: minimum x-coordinate for points
    :param max_x: maximum x-coordinate for points
    :param min_y: minimum y-coordinate for points
    :param max_y: maximum y-coordinate for points
    """
    points: Set[Point] = set()
    while len(points) < num_points:
        points.add((randint(min_x, max_x), randint(min_y, max_y)))
    return list(points)

def naive_convex_hull(points: List[Point]) -> List[Point]:
    """
    Naive O(n^3) convex hull algorithm for comparison.
    Uses brute force to find all points on the hull.
    """
    if len(points) <= 3:
        return base_case_hull(points)
    
    hull_points = []
    n = len(points)
    
    # Find all points that are on the convex hull
    for i in range(n):
        for j in range(n):
            if i == j:
                continue
            
            # Check if all other points are on one side of line from points[i] to points[j]
            left_side = 0
            right_side = 0
            
            for k in range(n):
                if k == i or k == j:
                    continue
                
                # Calculate cross product to determine which side of line point k is on
                cross = ((points[j][0] - points[i][0]) * (points[k][1] - points[i][1]) - 
                        (points[j][1] - points[i][1]) * (points[k][0] - points[i][0]))
                
                if cross > 0:
                    left_side += 1
                elif cross < 0:
                    right_side += 1
            
            # If all points are on one side, this edge is on the convex hull
            if left_side == 0 or right_side == 0:
                if points[i] not in hull_points:
                    hull_points.append(points[i])
                if points[j] not in hull_points:
                    hull_points.append(points[j])
    
    return hull_points

def run_benchmarks():
    # TODO: Generate points randomly, run your convex hull function,
    #  and record the time it takes on inputs of different sizes.
    # TODO: Plot a graph of runtime vs input size. What can you infer from the shape?
    sizes: List[int] = list(range(100, 2_000, 200))  # Reduced range for naive algorithm
    dnc_hull_times: List[float] = list()
    naive_hull_times: List[float] = list()
    
    for n in sizes:
        print(f'n: {n},', end=' ')
        points = generate_points(n)
        
        # Time divide-and-conquer algorithm
        start_time = time.time()
        # TODO: call compute_hull here
        hull_dnc = compute_hull(points)
        time_taken = time.time() - start_time  # time taken (in seconds) for divide-and-conquer
        print(f'dnc_time_taken: {time_taken:.3f},', end=' ')
        dnc_hull_times.append(time_taken)
        
        # Time naive algorithm
        start_time = time.time()
        # TODO: call base_case_hull here
        hull_naive = naive_convex_hull(points)
        time_taken = time.time() - start_time  # time taken (in seconds) for naive
        print(f'naive_time_taken: {time_taken:.3f}')
        naive_hull_times.append(time_taken)
    
    # Plot the results
    plt.figure(figsize=(10, 6))
    plt.scatter(sizes, dnc_hull_times, c='blue', alpha=0.7)
    plt.plot(sizes, dnc_hull_times, c='blue', label='Divide-and-Conquer O(n log n)')
    plt.scatter(sizes, naive_hull_times, c='red', alpha=0.7)
    plt.plot(sizes, naive_hull_times, c='red', label='Naive Algorithm O(n³)')
    plt.legend()
    plt.xlabel('Number of Input Points')
    plt.ylabel('Runtime (seconds)')
    plt.title('Convex Hull Algorithm Performance Comparison')
    plt.grid(True, alpha=0.3)
    plt.savefig('benchmark_plot.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Print analysis
    print("\nAnalysis:")
    print("From the graph, we can infer:")
    print("1. The divide-and-conquer algorithm shows O(n log n) growth - nearly linear")
    print("2. The naive algorithm shows O(n³) growth - cubic, much steeper curve")
    print("3. For larger inputs, the divide-and-conquer algorithm is significantly faster")
    print("4. The performance gap widens dramatically as input size increases")
    
    return

if __name__ == '__main__':
    run_benchmarks()